...
vix/tests/ratchet/128-progressive-tree.vix:11 - NOTE B resolved by binding `producer` and `consumer`; `consumer` is the demanded subfile text value, not an explicit stage-two process. RESOLVED: `finished_before(first, second)` accepts any two let-bound published values — value-vs-process included — and compares their `Completed`-event sequences from the frozen run log; both completions must exist and the first must strictly precede the second.
vix/tests/ratchet/128-progressive-tree.vix:10 - Adapted to the settled capability-package design (machine.primitive.command-package): product readiness is the declared output protocol's authority, never a filesystem poll, so the capability became the announcing `ProgressiveSh` and the script explicitly prints `vix-ready\tout/early.txt`. Under plain `Sh` the subfile is served only at process completion and the ordering claim would rightly fail.
vix/tests/ratchet/130-kill-when-satisfied.vix:1 - DEFERRED, not wired: three surfaces are missing. (a) The `killed(binding)` yielded check does not exist. (b) Kill-when-satisfied policy does not exist in the runner: the frontier unconditionally waits on every effect island's completion, and although `Runtime::abandon_demand` exists and cancels demand-owned effect tickets, nothing decides that a running effect's remaining products are unwanted and abandons it. (c) Ticket cancellation must be shown to terminate the child process (the exec backend's kill path), which is unverified. Wiring the rung before those land would fake the semantics.
