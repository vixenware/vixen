...
- ~~**The build order keys on package NAME alone.**~~ **CLOSED.** It keys on
  `PackageId { name, version }` now. A real lock may hold one name at two
  versions, and its `dependencies` entries then read `"name version"` — keyed on
  the name the duplicates collapsed in the done-set AND stopped matching those
  entries, so the Kahn walk came up short and the truncation read as a cycle.
  `trees/dup-name-crate` is that lock, generated by cargo rather than
  hand-written (`bitflags` at 1.3.2 and 2.13.1, via `package = "bitflags"`
  renames), and it came back two-of-three before the fix. The bare form is
  RESOLVED against the lock rather than parsed, because "this name is unique
  here" is exactly what writing it bare asserts; a bare entry naming zero
  ...
  file does not make yet (`--out-dir .`). The identity to key it on now exists.
