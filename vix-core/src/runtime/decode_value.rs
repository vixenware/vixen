//! Decoding a `decode(document, Format)` request into a wire [`PrimitiveValue`].
//!
//! This is the language-level half of the `decode` primitive: it reads the wire
//! request, runs the in-tree [`crate::decode`] parser against the requested
//! target type, and rebuilds the result as a `PrimitiveValue`. It touches only
//! the language (`crate::decode`/`crate::vir`) and the raw effect protocol, so it
//! lives in the language crate — `vixen_primitives::DecodePrimitive` is the thin
//! `RawPrimitive` registration wrapper over it.
...
use super::{
    EffectCtx, PrimitiveField, PrimitiveFieldValue, PrimitiveMachineError, PrimitiveValue,
    PrimitiveValueBody, ReadProjection, ValueId,
};

/// Read and decode a `decode`/`try_decode` wire request into its result
/// `PrimitiveValue` (the caller interns it). `fallible` requests wrap the result
/// (or a `DecodeError`) in a `Result` variant.
